...
if ($result->num_rows > 0) {
    ...
    $nomeUsuario = htmlspecialchars($row["nome"] ?? ''); // Escapando caracteres especiais
    $emailUsuario = htmlspecialchars($row["email"] ?? ''); // Escapando caracteres especiais
    $avatarPath = htmlspecialchars($row["avatar"] ?? 'default-avatar.png'); // Caminho da imagem do avatar
} else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["avatar"]) && $_FILES["avatar"]["error"] == UPLOAD_ERR_OK) {
        ...
        if (!is_dir($target_dir)) {
            mkdir($target_dir, 0755, true); // Cria o diretório com permissões
        }
        ...
        // Se não houver upload de imagem, apenas atualiza nome e email
        $stmt = $conn->prepare("UPDATE usuarios SET nome = ?, email = ? WHERE id = ?");
        $stmt->bind_param("ssi", $nome, $email, $id);
    }

    if ($stmt->execute()) {
        header("Location: perfil.php"); // Redireciona para o painel
        exit();
    } else {
